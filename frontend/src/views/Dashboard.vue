...
const allBrands = ref([])
const uniqueBrands = computed(() => allBrands.value)

const loadAllBrands = async () => {
  try {
    const response = await dashboardAPI.listHardware({ skip: 0, limit: 100 })
    const brands = new Set(response.data.items.map(hw => hw.brand))
    allBrands.value = Array.from(brands).sort()
  } catch (err) {
